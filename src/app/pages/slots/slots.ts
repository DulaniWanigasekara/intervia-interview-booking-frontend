...
import { BookModalComponent } from '../../shared/components/book-modal/book-modal';
...
@Component({
  selector: 'app-slots',
  standalone: true,
  imports: [CommonModule, StatusBadgeComponent, BookModalComponent],
...
export class SlotsComponent implements OnInit {
  ...
  onSlotClick(slot: Slot) {
    ...
    if (!localStorage.getItem('intervia_candidate')) {
  ...
  confirmBooking() {
    ...
    this.bookingService.create({
      ...
      error: (err) => {
        this.bookingLoading = false;
        this.bookingError = err.error?.message || 'Booking failed. Try again.';
      }
    });
  }

  slotCardClass(slot: Slot): string {
    if (this.isMySlot(slot))
      return 'border-green-400 cursor-default';
    if (slot.status === 'AVAILABLE')
      return 'border-gray-200 cursor-pointer hover:border-indigo-400 hover:shadow-lg hover:-translate-y-0.5';
    return 'border-gray-100 cursor-default opacity-60';
  }

  topBarClass(slot: Slot): string {
    if (this.isMySlot(slot)) return 'from-green-400 to-emerald-400';
    if (slot.status === 'AVAILABLE') return 'from-blue-400 to-indigo-500';
    if (slot.status === 'BOOKED') return 'from-gray-300 to-gray-400';
